...
const DraftSales = ({ invoice }) => {
  ...
  useEffect(() => {
    ...
    if (formData.discountType === 'percentage') {
      discountedPrice = productPrice * (1 - (formData.discount || 0) / 100);
    }
    if (formData.discountType === 'fixed') {
      discountedPrice = productPrice - (formData.discountRs || 0);
    }
  ...
  const removeRow = async (index) => {
    ...
    const invoiceProductId = rowToRemove[9];

    if (invoiceProductId) {
      try {
        const response = await fetch(`${config.BASE_URL}/invoiceProducts/${invoiceProductId}`, {
          method: 'DELETE',
        });
        ...
        console.log('Product deleted from the database:', invoiceProductId);
      } catch (error) {
        console.error('Error deleting product from the database:', error);
        alert('Failed to delete the product from the database. Please try again.');
        return;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const row of tableData) {
        ...
        const existingProductResponse = await fetch(`${config.BASE_URL}/invoiceProducts/${invoiceResult.invoiceId}/${row[7]}`);
        if (existingProductResponse.ok) {
          const existingProduct = await existingProductResponse.json();

          // Update existing product
          const updateResponse = await fetch(`${config.BASE_URL}/invoiceProducts/${existingProduct.id}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          console.log('Product updated:', existingProduct.id);
        } else {
          // Create new product
                ...
                <div className="row">
                  ...
                  {formData.discountType === 'percentage' && (
                    <div className="product-details col-md-3 mb-2">
                      ...
                      <input onChange={handleChange} value={formData.discount} type="number" onWheel={(e) => e.target.blur()} name="discount" className="form-control"
                        id="discount"
                  ...
                  {formData.discountType === 'fixed' && (
                    <div className="product-details col-md-3 mb-2">
                      ...
                      <input onChange={handleChange} value={formData.discountRs} type="number" onWheel={(e) => e.target.blur()} name="discountRs" className="form-control"
                        id="discountRs"
              ...
              <tbody>
                {tableData.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    ...
                    <td>
                      <p onClick={() => removeRow(rowIndex)} className='btn btn-danger btn-sm'>Remove</p>
                    </td>
                  </tr>
                ))}
              </tbody>
