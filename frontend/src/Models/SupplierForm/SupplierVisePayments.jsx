...
function SupplierVisePayments() {
    ...
    const [paidStatus, setPaidStatus] = useState(() => {
        const savedStatus = localStorage.getItem(`paidStatus-${supplierId}`);
    ...
    const fetchStockPayment = async (stockId) => {
        try {
            ...
            console.log("Fetched Stock Payments: ", stockPayments); // Check API response
            ...
            if (stockPayments.length > 0) {
                ...
                setPayStatus(prevStatus => {
                    ...
                    console.log("Updated Pay Status: ", updatedStatus); // Ensure payStatus is being updated correctly
    ...
    const handleShowPaidChange = (e) => setShowPaidInvoices(e.target.paid);
    ...
    const generatePDF = () => {
        ...
        const filteredDataForPDF = selectedDate
            ? groupedDataWithBalance.filter(([date]) => {
                const [dd, mm, yyyy] = date.split('/');
                return `${yyyy}-${mm}-${dd}` === selectedDate;
            })
        ...
        const paidFilteredDataForPDF = filteredDataForPDF.filter(([date, group]) => {
            const isPaid = paidStatus[date] || false;
                ...
                <div className="mt-4">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingRight: "20px" }}>
                        <div style={{ paddingLeft: "10px", width: "100%" }}>
                            ...
                            <div className='row' style={{ display: 'grid', gridTemplateColumns: 'auto 1px 1fr', gap: '1rem', marginBottom: '1.5rem', justifyContent: 'center', placeItems: 'center' }}>
